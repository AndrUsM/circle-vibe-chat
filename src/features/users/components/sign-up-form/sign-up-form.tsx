...
export const SignUpForm: React.FC = () => {
	...
	>
		{({ values, setFieldValue }: FormikProps<SignUpFormInput>) => (<StackLayout space="0.75rem">
			...
			<FileUploadForm
				...
				label="Avatar" bucket={ConversationBucketNameEnum.USER_AVATARS}
				type={FileUploadFormFileType.IMAGE}
				afterUpload={(fileUrls) => {
					const avatarUrl = (fileUrls as UploadImageOutputDto)?.optimisedFilePath ?? fileUrls?.filePath;
			...
			<FormGroup label="User Type" formFieldName="type">
				<FormControlSelect>
					{USER_TYPE_DROPDOWN_OPTIONS.map(({ key, label }) => (<option value={key}>{t(label)}</option>))}
				</FormControlSelect>
			</FormGroup>
			...
			<ClusterLayout>
				<FormGroup isRequired label="Country" formFieldName="country">
					<FormControlSelect>
						{countryDropdownOptions.map(({ code, label }) => (<option value={code}>{t(label)}</option>))}
					</FormControlSelect>
				</FormGroup>
			...
			<StackLayout space={"1rem"}>
				...
				<CenteredVertialLayout space={"1rem"} justifyContent="center">
					...
					<Button color="secondary" onClick={onNavigateToRestorePasswordPage}>{t(
						'login.restore-password.button')}</Button>
				</CenteredVertialLayout>
			</StackLayout>
		</StackLayout>)}
	</Form>);

...
export const MessagesFilterBar: React.FC<MessagesFilterBarProps> = ({
	conversationId, initialValues, onClose = noop, onSubmit,
}) => {
	...
	return (<Form
		initialValues={{
			...MESSAGES_FILTER_BAR_FORM_INITIAL_VALUES, ...initialValues,
		}}
	...
	>
		...
		}: FormikProps<MessagesFilterBarFormValues>) => (<StackLayout space="0.25rem">
			...
			<Show.When isTrue={chatParticipants.length > 1}>
				<FormGroup label="Participants:" formFieldName="senderIds">
					<StackLayout space="0.25rem">
						{chatParticipants.map((participant) => (<CenteredVertialLayout
							key={participant.id}
						...
						>
                          ...
                          <span
	                          className={classNames({
		                          "font-semibold": currentConversationParticipant?.id === participant.id,
	                          })}
                          >

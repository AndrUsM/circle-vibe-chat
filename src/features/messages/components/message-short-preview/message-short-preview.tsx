...
export const MessageShortPreview: ExtendedReactFunctionalComponent<MessageShortPreviewProps> = ({
  message,
}) => {
    ...
    >
      ...
      <Show.Else>
        ...
        <Show.When isTrue={Boolean(sender?.user)}>
          <span className='truncate white-space-nowrap font-semibold'>- {senderFullName}</span>
        </Show.When>
      </Show.Else>
    </ClusterLayout>
  );
